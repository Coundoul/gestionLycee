...
import mefpai.gouv.sn.service.ApprenantService;
...
/**
 * REST controller for managing {@link mefpai.gouv.sn.domain.Apprenant}.
 */
@RestController
@RequestMapping("/api")
public class ApprenantResource {
    ...
    public ResponseEntity<List<Apprenant>> getAllApprenants(Pageable pageable) {
        log.debug("REST request to get a page of Apprenants");
        if (SecurityUtils.hasCurrentUserThisAuthority("ROLE_ADMIN")) {
            Page<Apprenant> page = apprenantService.findAll(pageable);
        ...
        return ResponseEntity
            ...
            .headers(
                PaginationUtil.generatePaginationHttpHeaders(
                    ...
                    (apprenantService.findByUserIsCurrentUser(pageable))
                )
            )
            .body((apprenantRepository.findByUserIsCurrentUser(pageable)).getContent());
    }
