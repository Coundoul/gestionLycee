...
export class BesoinComponent implements OnInit {
  ...
  trackId(index: number, item: IBesoin): number {
    return item.id!;
  }
  ...
  delete(besoin: IBesoin): void {
    const modalRef = this.modalService.open(BesoinDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.besoin = besoin;
  ...
  protected onSuccess(data: IBesoin[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
